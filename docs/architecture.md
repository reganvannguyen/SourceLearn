...
graph TD
    ...
    subgraph Database ["Data & Storage Layer"]
        ...
        FileStore[("Local Disk Storage (/uploads)")]
    end
...
- `id` (Integer, Primary Key, autoincrement)
- `document_id` (Integer, Foreign Key &rarr; `documents.id` with `CASCADE` delete)
...
### Why PostgreSQL + `pgvector` Instead of External Vector DBs?
 многих applications use standalone vector databases (e.g., Pinecone, Milvus, Qdrant). For SourceLearn, **`pgvector`** co-located inside PostgreSQL was chosen deliberately:

1. **Transactional Consistency (ACID)**: When a user deletes a notebook or document, foreign key constraints (`ON DELETE CASCADE`) guarantee that all associated embeddings, text chunks, and chat messages are cleaned up atomically. There is zero risk of orphaned vector embeddings or desynchronized state.
